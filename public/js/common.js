...
$(document).ready(function () {
    ...
    jQuery.validator.addMethod("abs_digit", function (value, element) {
        return (
            ...
            Number.isInteger(Math.abs(__number_uf(value)))
        );
    });
    ...
    var buttons = [
        ...
        // },
        {
            extend: "csv",
            text:
                '<i class="fa fa-file-csv" aria-hidden="true"></i> <span class="btn-text"> ' +
                LANG.export_to_csv +
                "</span>",
        ...
        },
        {
            extend: "excel",
            text:
                '<i class="fa fa-file-excel" aria-hidden="true"></i><span class="btn-text"> ' +
        ...
        },
        {
            extend: "print",
            text:
                '<i class="fa fa-print" aria-hidden="true"></i> <span class="btn-text"> ' +
                LANG.print +
                "</span>",
        ...
        },
        {
            extend: "colvis",
            text:
                '<i class="fa fa-columns" aria-hidden="true"></i> <span class="btn-text"> ' +
                LANG.col_vis +
                "</span>",
            className: "btn btn-sm buttons-html5",
        },
    ...
    var pdf_btn = {
        extend: "pdf",
        text:
            '<i class="fa fa-file-pdf" aria-hidden="true"></i> <span class="btn-text"> ' +
            LANG.export_to_pdf +
            "</span>",
        className: "btn btn-sm buttons-html5",
        exportOptions: {
    ...
    var observer = new MutationObserver(function () {
        ...
        if (processingElem) {
            processingElem.innerHTML =
                '<img src="' + getLoaderImageUrl() + '">';
        }
    });
...
function check_window_width() {
    ...
    var sidebar_collapse =
        localStorage.getItem("bpos_sidebar_collapse") || "false";
...
$("table#stock_adjustment_product_table tbody").on(
    "change",
    "select.lot_number",
    function () {
        ...
        if ($(this).val()) {
            ...
            if (qty_available_el.length) {
                qty_available_el.text(
                    __currency_trans_from_en(default_qty, false)
                );
            }
        }
...
$(document).on("change", ".cash_denomination", function () {
    ...
    table.find("tbody tr").each(function () {
        var denomination = parseFloat(
            $(this).find(".cash_denomination").attr("data-denomination")
        );
        ...
        $(this)
            .find("span.denomination_subtotal")
            .text(__currency_trans_from_en(subtotal, true));
    });

    table
        .find("span.denomination_total")
