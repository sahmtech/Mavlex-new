$(document).ready(function () {
    function validate_files(files) {
        ...
        var allowed_extensions = $.map(
            window.mediaUploadConfig.allowedExtensions,
            function (ext) {
                return ext.toLowerCase();
            }
        );
        var allowed_mime_types = $.map(
            window.mediaUploadConfig.allowedMimeTypes,
            function (mime) {
                return mime.toLowerCase();
            }
        );
        var max_file_size =
            window.mediaUploadConfig.document_size_limit || 5000000;
    ...
    function initialize_file_validation() {
        $('input[type="file"], .docus_notes_media').on(
            "change",
            function (event) {
    ...
    contact_table = $("#contact_table").DataTable({
        ...
        ajax: {
            ...
            data: function (d) {
                ...
                if (
                    $("#has_sell_due").length > 0 &&
                    $("#has_sell_due").is(":checked")
                ) {
                    d.has_sell_due = true;
                }

                if (
                    $("#has_sell_return").length > 0 &&
                    $("#has_sell_return").is(":checked")
                ) {
                    d.has_sell_return = true;
                }

                if (
                    $("#has_purchase_due").length > 0 &&
                    $("#has_purchase_due").is(":checked")
                ) {
        ...
        footerCallback: function (row, data, start, end, display) {
            ...
            $(".footer_contact_return_due").html(
                __currency_trans_from_en(total_return_due)
            );
        },
    });
    ...
    if ($("form#bussiness_edit_form").length > 0) {
        ...
        $("input#purchase_in_diff_currency").on(
            "ifUnchecked",
            function (event) {
                $(
                    "div#settings_purchase_currency_div, div#settings_currency_exchange_div"
    ...
    $(document).on(
        "change",
        "input[type=radio][name=scheme_type]",
        function () {
    ...
    expense_table = $("#expense_table").DataTable({
        ...
        fnDrawCallback: function (row, data, start, end, display) {
            ...
            $(".footer_expense_total").html(
                __currency_trans_from_en(expense_total)
            );
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        if (result.success == true) {
                            ...
                            if (
                                typeof get_recent_transactions !== "undefined"
                            ) {
    ...
    $(document).on(
        "keyup",
        "form#unit_add_form input#actual_name",
        function () {
    ...
    $(document).on(
        "keyup",
        "form#unit_edit_form input#actual_name",
        function () {
...
function show_invoice_preview() {
    if ($("input[type=radio][name=scheme_type]:checked").val() == "blank") {
        ...
        $("input[type=radio][name=scheme_type]:checked").val() == "year"
    ) {
        var d = new Date();
...
$(document).on(
    "show.bs.modal",
    ".register_details_modal, .close_register_modal",
    function () {
...
function calculateExpensePaymentDue() {
    ...
    var payment_amount = __read_number(
        $("#add_expense_modal_form input.payment-amount")
    );
    var payment_due = final_total - payment_amount;
    $("#expense_payment_due").text(
        __currency_trans_from_en(payment_due, true, false)
    );
}
...
$(".sms_setting_add_modal, .sms_setting_edit_modal").on(
    "shown.bs.modal",
    function (e) {
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.success == 1) {
                ...
                $(
                    `#invoice_discount_type option[value='${result.data.default_discount_type}']`
                ).prop("selected", true);
                $(
                    `#invoice_tax_rate option[value='${result.data.tax_rate_id}']`
                ).prop("selected", true);
            ...
            success: function (result) {
                if (result.success === true) {
                    ...
                    if (result.note.description) {
                        ...
                        var edit_url =
                            "/invoice-notes/" + result.note.id + "/edit";
                        var surname =
                            result.note.created_user.surname !== null
                                ? result.note.created_user.surname
                                : "";
                        var first_name =
                            result.note.created_user.first_name !== null
                                ? result.note.created_user.first_name
                                : "";
                        var last_name =
                            result.note.created_user.last_name !== null
                                ? result.note.created_user.last_name
                                : "";
                        var html = "<tr>";
                        html +=
                            "<td>" + surname + first_name + last_name + "</td>";
                        ...
                        html +=
                            ...
                            edit_url +
                            ' class="edit_invoice_note">';
                        html += "<img src='/img/icons/edit.svg' alt='' />";
                        html += "</button>";
                        html +=
                            '<button type="button" data-href=' +
                            delete_url +
                            ' class="delete_invoice_note">';
            ...
            success: function (result) {
                if (result.success === true) {
                    ...
                    if (result.reminder.description) {
                        var surname =
                            result.reminder.created_user.surname !== null
                                ? result.reminder.created_user.surname
                                : "";
                        var first_name =
                            result.reminder.created_user.first_name !== null
                                ? result.reminder.created_user.first_name
                                : "";
                        var last_name =
                            result.reminder.created_user.last_name !== null
                                ? result.reminder.created_user.last_name
                                : "";
                        var html = "<tr>";
                        ...
                        if (result.reminder.status == 1) {
                            html +=
                                '<td><span class="label bg-light-green">Sent</span></td>';
                        } else {
                            html +=
                                '<td><span class="label bg-yellow">Not Sent</span></td>';
                        }
                        html += "</tr>";
                        $("#table_data").prepend(html);
