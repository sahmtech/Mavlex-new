...
class BusinessController extends Controller
{
    ...
    public function getBusinessSettings(Request $request)
    {
        ...
        return view('business.' . $request->segment(2), compact('business', 'business_locations', 'currencies', 'tax_rates', 'timezone_list', 'months', 'accounting_methods', 'commission_agent_dropdown', 'units_dropdown', 'barcodes_dropdown', 'date_formats', 'shortcuts', 'pos_settings', 'dashboard_settings', 'restaurant_settings', 'modules', 'theme_colors', 'email_settings', 'sms_settings', 'mail_drivers', 'allow_superadmin_email_settings', 'custom_labels', 'common_settings', 'weighing_scale_setting', 'primary_color', 'secondary_color'));
    }
    ...
    public function postBusinessSettings(Request $request)
    {
        ...
        try {
            ...
            } elseif (!empty($request->input('enable_rp')) && $request->input('enable_rp') == 0) {
            ...
            } elseif (!empty($request->input('enable_tooltip')) && $request->input('enable_tooltip') == 0) {
            ...
            } elseif ($request->has('is_product_setting')) {
            ...
            } elseif ($request->input('is_purchase_setting') == 1) {
                $checkboxes = [
            ...
            } elseif ($request->has('is_stock_transfer_setting')) {
                $checkboxes = [
                    'es_unit_price',
                    'es_sub_total',
                    'es_net_total_amount',
            ...
            } elseif ($request->input('is_pos_setting') == 1) {
                // code...
            }
            ...
            $pre_pos_setting = json_decode(
                optional($pre_busines_detail)->pos_settings,
                true
            ) ?? [];
            ...
            for ($i = 1; $i <= 10; $i++) {
                ...
                if ($request->boolean($removeKey)) {
                    if (!empty($pre_pos_setting[$key])) {
                        @unlink(public_path('uploads/carousel_images/' . $pre_pos_setting[$key]));
                    }
                ...
                if ($request->hasFile($key)) {
                    ...
                    $pos_settings[$key] = $this->businessUtil
                        ->uploadFile($request, $key, 'carousel_images', 'image');
                    continue;
                }
            }
            foreach ($this->businessUtil->defaultPosSettings() as $k => $v) {
